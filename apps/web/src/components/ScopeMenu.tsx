...
const roleLabels: Record<Scope["role"], string> = {
  owner: "Owner",
  admin: "Admin",
  member: "Member",
};
  ...
  return (
    ...
    >
      <section className="scope-menu__section">
        ...
        {canCreatePersonalSpace ? (
          ...
          <p className="scope-menu__upgrade caption">
            Personal space limit reached. Upgrade for more.
          </p>
        )}
      </section>
      ...
      <section className="scope-menu__section">
        ...
        <ul className="scope-menu__list">
          {teamScopes.length === 0 ? (
            <li className="scope-menu__empty caption">No team spaces yet.</li>
          ) : (
            teamScopes.map((scope) => (
              <li key={scope.id}>
                ...
                >
                  ...
                  <span className="scope-menu__item-main">
                    <span className="scope-menu__item-name">{scope.name}</span>
                    <span className="scope-menu__item-meta">{roleLabels[scope.role]}</span>
                  </span>
                  {activeScopeId === scope.id && (
                    <Check size={16} strokeWidth={1.75} className="scope-menu__check" />
                  )}
        ...
        {canCreateTeamSpace ? (
          ...
          <p className="scope-menu__upgrade caption">
            Upgrade to Team plan to create team spaces.
          </p>
        )}
      </section>
