...
**Status:** complete (transport slices 1–4 shipped; slice 5 skipped; slice 6 UAT recorded July 27, 2026)
...
| 6 Verification pass | PASS (partial on conflict) | See manual UAT table below |

### Manual UAT (slice 6) — July 27, 2026

| Test | Result | Notes |
|------|--------|-------|
| Idle 3 tabs, 2 min | **PASS** | No `falling back to REST` spam |
| Live typing A→B→C | **PASS** | Propagation acceptable |
| Offline overlap + second cycle | **PARTIAL** | Conflict UI works for first conflict; **multi-conflict bug** — see TD-001 below |
| Reconnect after DevTools offline | **PASS** | Cursors return; no doc blanking; no reconnect race |
| `pnpm test:offline` | **PASS** | 70/70 (at implementation time) |
| Unit test (slice 3) | **PASS** | 4 transport tests |

### Issues found (deferred — not in scope for 09.1)

These are **pre-existing conflict-quality issues**, not regressions from transport changes. Tracked in [docs/32-technical-debt.md](../docs/32-technical-debt.md); fix scheduled in [09.2-offline-conflict-quality.md](09.2-offline-conflict-quality.md) **after M1b crypto**.

| ID | Summary |
|----|---------|
| **TD-001** | **Multi-conflict misclassification:** Block 1 edit-vs-remove correct; Block 2 same-word edit by A (offline) vs C (online) incorrectly shown as “User C removed Block 2”. Likely Phase 09 conflict clustering leftover. |
| **TD-002** | **Debug console noise:** `[offline-patch]`, `[peer-edit-contributions]`, `[block-audit]`, provider reconnect logs still print during conflict/reconnect. Remove when conflict flow is stable. |
...
- [09.2-offline-conflict-quality.md](09.2-offline-conflict-quality.md) — post-M1b fix for TD-001 / TD-002 found in slice 6 UAT
- [docs/32-technical-debt.md](../docs/32-technical-debt.md) — technical debt register
